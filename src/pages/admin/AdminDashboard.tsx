...
export default function AdminDashboard() {
  ...
  const kpiCards = [
    ...
    { label: 'Total Partners',  value: stats.totalDps,         icon: Bike,         bg: 'rgba(245,197,66,0.15)',    color: '#F5C542' },
    ...
    { label: 'Live Orders',     value: stats.liveOrders,       icon: Activity,     bg: 'rgba(245,197,66,0.15)',    color: '#F5C542' },
  ...
  return (
    <div className="p-4 md:p-8" style={{ background: '#050505', minHeight: '100%' }}>
      <div className="mb-7 flex items-center justify-between animate-fade-in-up">
        <div>
          <p className="text-[11px] font-extrabold uppercase tracking-[0.16em]" style={{ color: '#F5C542' }}>Operations</p>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={exportReport}
            ...
            style={{ background: 'rgba(245,197,66,0.15)', border: '1px solid rgba(245,197,66,0.25)', color: '#F5C542' }}>
      ...
      <div className="mb-5 grid grid-cols-1 gap-3 md:grid-cols-2">
        ...
        <div className="relative overflow-hidden rounded-3xl p-5 animate-slide-up"
          style={{ background: 'linear-gradient(135deg,rgba(245,197,66,0.15),rgba(245,197,66,0.07))', border: '1px solid rgba(245,197,66,0.2)', animationDelay: '360ms' }}>
          <div className="flex items-center gap-2 mb-2">
            <TrendingUp size={16} style={{ color: '#F5C542' }} />
            <p className="text-xs font-semibold" style={{ color: '#F5C542' }}>Month Revenue</p>
          </div>
            ...
            <div className="space-y-2">
              {topDps.map((dp, i) => (
                <div key={i} className="flex items-center justify-between rounded-2xl px-3 py-2.5"
                  style={{ background: i === 0 ? 'rgba(245,197,66,0.08)' : 'rgba(255,255,255,0.03)' }}>
                  <div className="flex items-center gap-2.5">
                    <span className="flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold"
                      style={i === 0 ? { background: '#F5C542', color: '#0B0B0B' } : { background: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.5)' }}>
                      {i + 1}
                    </span>
                    <span className="text-sm font-medium text-white">{dp.name}</span>
                  </div>
                  <span className="text-sm font-bold" style={{ color: '#F5C542' }}>{dp.deliveries} <span className="text-xs font-normal" style={{ color: 'rgba(255,255,255,0.4)' }}>orders</span></span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="fixed inset-0 z-50 animate-fade-in" style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)' }} onClick={() => setShowNotifPanel(false)}>
          <div className="absolute right-0 top-0 h-full w-full max-w-md overflow-y-auto animate-slide-in-right"
            ...
            <div className="sticky top-0 z-10 flex items-center justify-between px-5 py-4"
              ...
              <div className="flex items-center gap-2">
                <Bell size={18} style={{ color: '#F5C542' }} />
              ...
              <div className="flex items-center gap-2">
                {unreadCount > 0 && <button onClick={markAllRead} className="text-xs font-semibold" style={{ color: '#F5C542' }}>Mark all read</button>}
                ...
                return (
                  <div key={n.id} onClick={() => markRead(n.id)} className="flex gap-3 rounded-2xl p-3.5 cursor-pointer transition-all"
                    style={n.is_read
                      ...
                      : { background: 'rgba(245,197,66,0.07)', border: '1px solid rgba(245,197,66,0.18)' }}>
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl"
                      style={n.is_read ? { background: 'rgba(255,255,255,0.06)' } : { background: 'rgba(245,197,66,0.15)' }}>
                      <Icon size={16} style={{ color: n.is_read ? 'rgba(255,255,255,0.35)' : '#F5C542' }} />
                    </div>
                    <div className="flex-1 min-w-0">
...
function AdvanceAnalytics() {
  ...
  const cards = [
    { label: "Today's Scheduled", value: advanceStats.todayCount, icon: CalendarClock, color: '#F5C542' },
    ...
    { label: 'Completed', value: advanceStats.completed, icon: CheckCircle, color: '#F5C542' },
  ...
  const revenueCards = [
    { label: 'Advance Revenue', value: advanceStats.revenue, icon: IndianRupee, color: '#F5C542' },
  ...
  return (
    <div className="mb-6">
      <div className="flex items-center gap-2 mb-4">
        <BarChart3 size={20} style={{ color: '#F5C542' }} />
      ...
      <div className="card p-5">
        ...
        <div className="space-y-3">
          {[
            ...
            { label: 'Completed', count: advanceStats.completed, color: '#F5C542' },
