    ...
    <div className="flex h-screen flex-col" style={{ background: pg.bg }}>
        ...
        <div className="flex-1 min-w-0">
          ...
          {otherTyping ? (
            <div className="flex items-center gap-1.5">
              <span className="text-xs font-medium" style={{ color: '#D4F000' }}>typing</span>
              <div className="flex gap-0.5 items-center">
                {[0, 150, 300].map(delay => (
                  <span key={delay} className="h-1 w-1 rounded-full animate-bounce" style={{ background: '#D4F000', animationDelay: `${delay}ms` }} />
                ))}
              </div>
            </div>
          ) : dpInfo ? (
            ...
            <p className="flex items-center gap-1 text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>
              {lastOwnMsg?.read_at ? <><CheckCheck size={11} style={{ color: '#D4F000' }} /> Seen</> : lastOwnMsg ? <><Check size={11} /> Delivered</> : 'Chat'}
            </p>
          )}
        </div>
        ...
        <div className="mx-auto max-w-md space-y-2.5">
          ...
          {!order && isUser && fullOrderData?.order_type !== 'advance' && (
            <div className="mb-4 rounded-2xl px-4 py-3 text-center text-xs font-medium animate-fade-in"
              style={{ background: 'rgba(212,240,0,0.08)', border: '1px solid rgba(212,240,0,0.2)', color: 'rgba(255,255,255,0.6)' }}>
            ...
            return (
              <div key={msg.id} className={`flex items-end gap-2 ${isOwn ? 'justify-end' : 'justify-start'} animate-slide-up`}>
                ...
                <div className={`max-w-[75%] rounded-2xl px-3.5 py-2.5 ${isOwn ? 'rounded-br-sm' : 'rounded-bl-sm'}`}
                  style={isOwn
                    ? { background: pg.lime, boxShadow: '0 4px 16px rgba(212,240,0,0.2)' }
                  ...
                  {msg.message_type === 'quotation' && msg.quotation_data && (
                    <div className="min-w-[240px] space-y-3">
                      <div className="flex items-center justify-center gap-2 pb-2 border-b" style={{ borderColor: isOwn ? 'rgba(0,0,0,0.12)' : 'rgba(255,255,255,0.1)' }}>
                        <FileText size={15} style={{ color: isOwn ? '#0B0B0B' : '#D4F000' }} />
                        <p className="text-sm font-bold tracking-wide" style={{ color: isOwn ? '#0B0B0B' : '#D4F000' }}>Quotation</p>
                      </div>
                        ...
                        {String(msg.quotation_data.items_summary || '').split('\n').map((line: string, i: number) =>
                          line.trim() ? (
                            <li key={i} className="flex items-start gap-2 text-sm" style={{ color: isOwn ? '#0B0B0B' : 'rgba(255,255,255,0.85)' }}>
                              <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full" style={{ background: isOwn ? '#0B0B0B' : '#D4F000' }} />
                              {line.trim()}
                            </li>
                          ) : null
                        )}
                      ...
                      {order && <p className="text-center text-sm font-bold" style={{ color: isOwn ? 'rgba(0,0,0,0.7)' : '#D4F000' }}>✓ Accepted</p>}
                    </div>
                  )}
                  ...
                  {msg.message_type === 'advance_payment' && msg.quotation_data && (
                    <div className="w-72 space-y-3 p-4 rounded-2xl"
                      style={{ background: isOwn ? 'rgba(0,0,0,0.08)' : 'rgba(212,240,0,0.06)', border: `1px solid ${isOwn ? 'rgba(0,0,0,0.15)' : 'rgba(212,240,0,0.2)'}` }}>
                      <div className="flex items-center gap-2">
                        <div className="flex h-8 w-8 items-center justify-center rounded-full" style={{ background: 'rgba(212,240,0,0.2)' }}>
                          <CreditCard size={16} style={{ color: '#D4F000' }} />
                        </div>
                        <div>
                          <p className="text-sm font-bold" style={{ color: isOwn ? '#0B0B0B' : '#D4F000' }}>Advance Booking Confirmation</p>
                      ...
                      <div className="space-y-1.5 text-xs">
                        ...
                        <div className="flex justify-between"><span style={{ color: isOwn ? 'rgba(0,0,0,0.5)' : 'rgba(255,255,255,0.5)' }}>Amount</span><span className="font-bold" style={{ color: '#D4F000' }}>{formatCurrency(msg.quotation_data.amount)}</span></div>
                      ...
                      {isUser && (msg.quotation_data.status === 'waiting' || msg.quotation_data.status === 'rejected') && msg.advance_payment_id && (
                        <button onClick={() => { setShowPaymentProof(msg.advance_payment_id); setAdvancePaymentData(msg.quotation_data) }}
                          ...
                          style={{ background: '#D4F000', color: '#0B0B0B' }}>
          ...
          {otherTyping && (
            <div className="flex items-end gap-2 justify-start animate-fade-in">
              ...
              <div className="flex items-center gap-1.5 rounded-2xl rounded-bl-sm px-4 py-3" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)' }}>
                {[0, 150, 300].map(delay => (
                  <span key={delay} className="h-2 w-2 rounded-full animate-bounce" style={{ background: '#D4F000', animationDelay: `${delay}ms` }} />
                ))}
              </div>
            </div>
          )}
      ...
      {isCompleted && isUser && !hasRated && (
        <div className="shrink-0 px-4 py-3" style={{ background: pg.bg, borderTop: `1px solid ${pg.line}` }}>
          <button onClick={() => setShowRating(true)} className="w-full gap-2 rounded-2xl py-3.5 text-sm font-extrabold transition-all active:scale-95"
            style={{ background: pg.lime, color: pg.limeText, boxShadow: '0 10px 28px rgba(212,240,0,0.28)' }}>
      ...
      {isCompleted && hasRated && (
        <div className="shrink-0 flex items-center justify-center gap-2 px-4 py-3" style={{ background: pg.bg, borderTop: `1px solid ${pg.line}` }}>
          <CheckCircle size={15} style={{ color: '#D4F000' }} />
          <p className="text-sm font-medium" style={{ color: '#D4F000' }}>Order completed & rated</p>
        </div>
      )}
          ...
          >
            ...
            {!recording && (
              <button type="button" onClick={handleSend}
                className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl active:scale-90"
                style={input.trim()
                  ? { background: pg.lime, boxShadow: '0 8px 20px rgba(212,240,0,0.3)' }
      ...
      {fullOrderData && !isCompleted && (
        <div className="fixed bottom-24 left-0 right-0 z-20 flex justify-center gap-2 px-4">
          <button onClick={() => navigate(isUser ? `/app/chat/${roomId}/order` : `/dp/chat/${roomId}/order`)}
            ...
            style={{ background: pg.lime, color: pg.limeText, boxShadow: '0 10px 28px rgba(212,240,0,0.35)' }}>
            <ClipboardList size={14} /> View Full Order
          </button>
          {!isUser && !order && fullOrderData?.order_type !== 'advance' && (
            <button onClick={() => setShowQuotation(true)}
              className="flex items-center gap-2 rounded-full px-4 py-2.5 text-xs font-extrabold shadow-lg transition-all active:scale-95"
              style={{ background: pg.surface2, color: pg.lime, border: `1px solid rgba(212,240,0,0.35)` }}>
  ...
  return (
    <div className="flex items-center gap-2.5 min-w-[140px]">
      <button onClick={toggle} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full transition-all active:scale-90"
        style={{ background: isOwn ? 'rgba(0,0,0,0.2)' : 'rgba(212,240,0,0.2)' }}>
        {playing
          ? <Pause size={14} style={{ color: isOwn ? '#0B0B0B' : '#D4F000' }} />
          : <Play size={14} style={{ color: isOwn ? '#0B0B0B' : '#D4F000' }} />}
      </button>
        ...
        {Array.from({ length: 16 }).map((_, i) => (
          <div key={i} className={`flex-1 rounded-full ${playing ? 'animate-pulse' : ''}`}
            style={{
              ...
              background: isOwn ? 'rgba(0,0,0,0.35)' : 'rgba(212,240,0,0.5)',
      ...
      <div className="w-full max-w-md rounded-t-3xl p-6 animate-slide-in-bottom" style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.08)' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="mb-2 flex justify-center gap-3">
          {[1, 2, 3, 4, 5].map(i => (
            <button key={i} onClick={() => setStars(i)} className="transition-transform active:scale-90">
              <svg width={40} height={40} viewBox="0 0 24 24" fill={i <= stars ? '#D4F000' : 'none'} stroke={i <= stars ? '#D4F000' : 'rgba(255,255,255,0.2)'} strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
              </svg>
            </button>
          ))}
        </div>
        <p className="mb-4 text-center font-semibold" style={{ color: '#D4F000' }}>{labels[stars]}</p>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => onSubmit(stars, review)} className="flex-1 btn font-bold rounded-xl py-3"
            style={{ background: '#D4F000', color: '#0B0B0B' }}>Submit</button>
        </div>
      </div>
    </div>
  )
    ...
    <div className="fixed inset-0 z-[150] flex items-end justify-center bg-black/50 backdrop-blur-sm animate-fade-in" onClick={onClose}>
      <div className="w-full max-w-md rounded-t-3xl glass bottom-sheet max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 pb-8 pt-4 space-y-4">
          <div className="flex items-center gap-2">
            <CreditCard size={20} style={{ color: '#D4F000' }} />
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleSend} disabled={sending || !amount}
              ...
              style={{ background: 'linear-gradient(135deg, #D4F000, #D4F000)', color: '#0B0B0B' }}>
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-end justify-center bg-black/50 backdrop-blur-sm animate-fade-in" onClick={onClose}>
      <div className="w-full max-w-md rounded-t-3xl glass bottom-sheet max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 pb-8 pt-4 space-y-4">
          <div className="flex items-center gap-2">
            <Upload size={20} style={{ color: '#D4F000' }} />
          ...
          <div>
            ...
            <label htmlFor="proof-file" className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-white/20 py-6 text-sm text-white/50 transition-all hover:border-[#D4F000] hover:text-[#D4F000]">
...
function AdvanceTaskSummary({ request, statusLabel }: { request: any; statusLabel: string }) {
  ...
  const Row = ({ icon, label, value }: { icon: any; label: string; value: string }) => (
    <div className="flex items-start gap-2 py-1.5">
      <div className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-lg" style={{ background: 'rgba(212,240,0,0.1)' }}>
        {icon}
      </div>
  ...
  return (
    <div className="shrink-0 px-4 pt-3 pb-1" style={{ background: 'rgba(212,240,0,0.04)', borderBottom: '1px solid rgba(212,240,0,0.12)' }}>
      <div className="mx-auto max-w-md">
        ...
        >
          <div className="flex items-center gap-2">
            <ClipboardList size={15} style={{ color: '#D4F000' }} />
            <span className="text-sm font-bold text-white">Advance Task Summary</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="rounded-full px-2 py-0.5 text-[10px] font-bold" style={{ background: 'rgba(212,240,0,0.15)', color: '#D4F000', border: '1px solid rgba(212,240,0,0.25)' }}>
              {statusLabel}
            </span>
        ...
        {expanded && (
          <div className="pb-3 pt-1 animate-fade-in">
            {category && <Row icon={<Tag size={12} style={{ color: '#D4F000' }} />} label="Category" value={category} />}
            {scheduledDate && <Row icon={<CalendarClock size={12} style={{ color: '#D4F000' }} />} label="Scheduled Date" value={scheduledDate} />}
            {scheduledTime && <Row icon={<Clock size={12} style={{ color: '#D4F000' }} />} label="Scheduled Time" value={scheduledTime} />}
            {pickup && <Row icon={<MapPin size={12} style={{ color: '#D4F000' }} />} label="Pickup Address" value={pickup} />}
            {delivery && <Row icon={<Navigation size={12} style={{ color: '#D4F000' }} />} label="Delivery Address" value={delivery} />}
            {hasDescription && <Row icon={<FileText size={12} style={{ color: '#D4F000' }} />} label="Task Description" value={request.description} />}
            {hasBudget && <Row icon={<IndianRupee size={12} style={{ color: '#D4F000' }} />} label="Budget" value={formatCurrency(Number(request.max_budget))} />}
            {hasInstructions && <Row icon={<ShieldCheck size={12} style={{ color: '#D4F000' }} />} label="Special Instructions" value={request.special_instructions} />}
            ...
            {hasVoice && (
              <div className="mt-2 flex items-center gap-2 rounded-xl px-3 py-2" style={{ background: 'rgba(212,240,0,0.06)' }}>
                <Volume2 size={14} style={{ color: '#D4F000' }} />
                <span className="text-xs" style={{ color: 'rgba(255,255,255,0.6)' }}>Voice note attached</span>
                <a href={request.voice_note_url} target="_blank" rel="noopener noreferrer" className="ml-auto text-xs font-semibold" style={{ color: '#D4F000' }}>Play</a>
              </div>
            )}

            <div className="mt-3 flex items-center gap-1.5 rounded-xl px-3 py-2" style={{ background: 'rgba(212,240,0,0.06)' }}>
              <CheckCircle size={12} style={{ color: '#D4F000' }} />
              <p className="text-[11px]" style={{ color: 'rgba(255,255,255,0.5)' }}>Current Status: <span className="font-bold" style={{ color: '#D4F000' }}>{statusLabel}</span></p>
            </div>
          </div>
        )}
      </div>
