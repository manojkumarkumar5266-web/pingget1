      ...
      <div className="relative flex-shrink-0" style={{ height: '40vh', minHeight: '280px' }}>
        {isPending ? (
          ...
          <VisualTracking
            ...
            eta={liveEta !== null ? (liveEta === 0 ? 'Arrived' : `${liveEta} min`) : etaLabel}
          />
        )}
      </div>
              ...
              <div className="mb-4 rounded-2xl border border-white/10 bg-white/5 p-4 animate-slide-up">
                <div className="flex items-center gap-3">
                  <div className="relative h-16 w-16 overflow-hidden rounded-2xl bg-white/5 shrink-0">
                    {dpProfile.photo_url ? (
                      <img src={dpProfile.photo_url + (dpProfile.photo_url.includes('?') ? '&' : '?') + 'cb=' + Date.now()} alt={dpProfile.full_name || ''} className="h-full w-full object-cover" />
                    ) : (
