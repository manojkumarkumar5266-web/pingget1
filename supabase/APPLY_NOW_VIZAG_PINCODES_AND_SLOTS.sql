...
-- 1) Advance booking hours: 5:00 AM – 11:00 PM
-- 2) Correct Visakhapatnam pincode area names (India Post)
...
SELECT 'Visakhapatnam', 10, true
WHERE NOT EXISTS (SELECT 1 FROM public.cities WHERE name = 'Visakhapatnam');
...
BEGIN
  ...
  IF viz_id IS NULL THEN
    RAISE NOTICE 'Visakhapatnam city missing';
    RETURN;
  END IF;
