-- Admin support chat (user/DP ↔ admin), separate from order chat_rooms
...
CREATE INDEX IF NOT EXISTS support_chats_admin_unread_idx ON support_chats (admin_unread) WHERE admin_unread > 0;
...
-- Open or reuse an open support chat for the current user/DP
...
-- Public DP stats for customers on tracking (delivery_partners RLS is own/admin only)
...
-- Realtime for support chat badges / live messages
DO $$
BEGIN
  BEGIN
